...
### v2.2.5 UI 收尾 + 圣域规则热修
- UI 收尾：
  - 完成角色操作页最后一轮精品化收口，统一顶部 hero、资源带、任务区块与卡片节奏。
  - 做装模拟页继续精修排版和指标卡节奏，让整套 UI 更接近统一工作台语言。
- 商店文案修正：
  - `未知键队挑战券(刻印)` 更正为 `未知缝隙挑战券(刻印)`。
  - `深渊重镇补充券(刻印)` 更正为 `深渊重铸补充券(刻印)`。
- 圣域规则回滚同步：
  - `圣域：卢德莱` 恢复为每周 `挑战 4 / 开箱 2`。
  - `圣域：侵蚀净化所` 恢复为每周 `挑战 4 / 开箱 2`。
  - 每次开箱默认消耗 `80` 奥德。
  - `深渊重铸补充券(刻印)` 绑定 `圣域：卢德莱`，提供 `+1 挑战 / +1 开箱`，并限定为同伺服器每周一次。
- 数据模型调整：
  - 圣域从简化的“每本单一剩余次数”恢复为“挑战/开箱双计数 + 卢德莱补充 bonus”。
  - 存档归一化兼容旧字段 `sanctumRaidRemaining / sanctumBoxRemaining`，避免旧档直接失效。
- 本次校验：
  - `npm run typecheck` 通过
  - `npm run test:unit` 通过
  - `npm run build` 通过
...
### v2.2.4 新赛季规则热修
- 新赛季规则同步：
  - 删除 `讨伐战` 全部相关任务、展示、统计与设置项。
  - `每日副本` 从“每角色 7 次”调整为“同伺服器共享每周 14 次”。
  - `每周指令书 / 深渊下层指令 / 深渊中层指令` 保持总次数不变，但改为同伺服器共享次数池。
  - `征服 / 超越` 每周副本奖励补充次数分别调整为 `14 / 7`。
...
### 复盘与开发手册补充“启动即静默更新”经验
- 更新文档：
  - `PROJECT_REVIEW.md`
  - `MY_DEV_PLAYBOOK.md`
  - `NEXT_PROJECT_START_CHECKLIST.md`
- 新增内容重点：
  - 把“启动先检查更新、静默安装、失败回退到旧版”总结为一个关键体验升级
  - 在开发手册中明确：更新策略应尽早设计，而不是临近发版时才临时决定
  - 在启动清单中增加“更新是手动检查还是启动先检查”“是否静默安装后再进主界面”等确认项

### v2.2.3 启动即静默更新
- 自动更新链路改造：
  - 启动时不再由主进程偷偷后台下载
  - 改为 App 首屏显式执行启动更新检查
  - 若发现新版本，先显示“正在更新到 vX.Y.Z...”
  - 下载完成后调用静默安装并自动重启
  - 若更新失败，则继续进入旧版，不阻塞使用
- IPC 增强：
  - 新增 `checkStartupAppUpdate`，与手动“检查更新”分开
- 冒烟与基线脚本适配：
  - 兼容新的启动更新屏和“创建第一个账号”空状态
- 本次校验：
  - `npm run typecheck` 通过
  - `npm run test:unit` 通过
  - `npm run build` 通过
  - `npm run smoke:ui` 通过
- 结论：
  - 安装版更新体验已从“后台下载、退出再装”升级为“启动先更新、更新后再进主界面”。
...
- 目的：
  - 把本项目的产品边界、架构演进、热修经验、发布流程、UI 收口方法和 AI 协作方法沉淀成可复用资产。
  - 为后续新项目提供一套稳定起步模板，而不是从零重新摸索。

### v2.2.2 做装模拟器增强（材料优先工作流）
- 做装模拟器结果区顺序重排：
  - `材料明细与库存修正`
  - `材料缺口与采购清单`
  - `模拟结论`
...
- 新增 `金价换算辅助`：
  ...
  - 两个输入互相推导
  - 支持交易行税后修正
  - 可直接估算补齐当前缺口大约还要收多少 RMB
- 本次校验：
  - `npm run typecheck` 通过
  - `npm run test:unit` 通过
  - `npm run build` 通过
- 结论：
  - 做装模拟器现在更符合真实操作顺序，先处理材料与缺口，再看利润与现金换算。

## 2026-03-09

### UI 改造 Phase 15 完成（总览语义强化 + 角色页顶部减重）
- 新增本轮任务计划：
  - [UI_PHASE15_TASK_PLAN.md](D:/Projects/AION2%20Dashboard/UI_PHASE15_TASK_PLAN.md)
  - 固化本轮目标、执行顺序与验收重点，避免后续微调时丢失方向。
- 角色概览总览语义强化：
  - `高优先 / 副本 / 周常 / 使命 / 休闲` 恢复浅色语义底。
  - 各任务行增加轻色背景和左侧强调条，不再只靠红字突出重点。
  - 每张角色卡增加“当前最需要处理”提示条，先给出单一重点。
- 角色操作页减重：
  - 顶部改成 `当前角色卡 + 角色工具卡` 两段式结构。
  - `周统计校准 / 角色资料 / 快捷操作` 合并进同一工具区，减少首屏卡片数量。
  - 任务卡改为 `标题 + 一行摘要 + 状态 + 操作` 的更扁平结构。
- 资源区续收：
  - `奥德能量 / 微风商店记录 / 变换记录` 保持关联布局不变，继续压缩垂直间距。
- 本次校验：
  - `npm run typecheck` 通过
  - `npm run build` 通过
  - `npm run test:smoke` 通过
- 结论：
  - 总览页重点识别明显提升。
  - 角色页首屏更快进入任务主内容，顶部管理区不再抢高度。

### UI Phase 15 微调（语义色压淡 + 角色页按钮层级统一）
- 总览卡语义色继续压缩：
  - 各分组底色和 chip 饱和度下调，避免“有颜色但太吵”。
  - 保留任务行左侧强调条，让重点仍然一眼可见。
- 角色页按钮层级统一：
  - `保存资料 / 保存校准` 作为主要动作，统一为深色紧凑按钮。
  - 其余操作保持浅色 pill，并统一最小宽度。
- 小屏继续收口：
  - 角色页卡片、任务区和工具区在窄屏下继续压缩内边距。
  - 角色页操作按钮在小屏改为更均匀的换行布局。
- 本次校验：
  - `npm run typecheck` 通过
  - `npm run build` 通过
  - `npm run test:smoke` 通过

### v2.1.4 发版准备（做装模拟 / 市场工具命名收口）
- 工坊导航命名调整：
  - `工坊` 统一改名为 `做装模拟`
  - `专业工具` 统一改名为 `市场工具`
- 用户可见文案同步：
  - 加载态、初始化失败、操作失败等兜底提示改为“做装模拟”口径
- 自动化校验同步：
  - `scripts/ui-smoke.mjs` 已改为定位 `做装模拟`
  - `scripts/ui-baseline-phase0-capture.mjs` 已改为定位 `做装模拟 / 市场工具`
- 本次校验：
  - `npm run typecheck` 通过
  - `npm run build` 通过
  - `npm run test:smoke` 通过
- 结论：
  - UI 命名已经和当前使用场景对齐，可直接进入 `v2.1.4` 打包发布。

### UI 改造 Phase 14 完成（总览细节回补 + 专业工具浅色化续收）
- 角色总览卡回补任务细节：
  - 从“高度摘要化”改为“分组行列表”。
  - 当前可执行项按 `高优先 / 副本 / 周常 / 使命 / 休闲` 展示。
  - 保留足够细节，避免总览失去实用性。
- 专业工具浅色化继续推进：
  - `OCR抓价器` 的主流程卡、高级设置和调试表已明显去黑底化。
  - `市场分析器` 的首屏控件、高级筛选和最近抓价表已改为浅色容器。
  - `库存管理` 与 `历史价格管理` 的表格、输入区和提示区已改为浅色 token 容器。
- 截图回归：
  - 新增命令：`npm run capture:ui-baseline:phase14`
  - Phase 14 截图目录：`artifacts/ui-baseline/phase14/`
- 本次校验：
  - `npm run typecheck` 通过
  - `npm run build` 通过
  - `node scripts/ui-baseline-phase0-capture.mjs --out-dir artifacts/ui-baseline/phase14` 通过（16 张截图）
- 结论：
  - 总览卡重新恢复“能看细节”的实用价值。
  - 专业工具内部仍可继续收尾，但整体观感已经开始统一。
...
- 精简折叠区文案：
  - `专业工具` 仅保留标题与展开/收起动作。
  - 删除“默认不打断做装主流程”这类重复说明。
- 回归：
  - `npm run typecheck` 通过
  - `node scripts/ui-baseline-phase0-capture.mjs --out-dir artifacts/ui-baseline/phase13` 通过

### UI 改造 Phase 10-13 完成（总览瘦身 + 单任务工坊 + 侧栏按页收起 + Token 收敛）
- 角色总览卡继续瘦身：
  - `快速录入` 改为折叠入口，不再常驻占满中栏首屏。
  - 角色卡默认仅展示 `3 个最高优先项目 + 1 个次级关注摘要`。
  - 筛选与排序改为单独控制条，减少“控件先于内容”。
- 工坊进入更明确的单任务模式：
  - 顶部更名为 `装备制作工作台`。
  - 默认首屏只保留“做装模拟器”。
  - `OCR / 市场分析 / 历史价格 / 库存` 全部收进 `专业工具` 折叠区。
- 侧栏按页面收起：
  - `设置页 / 工坊` 不再占用右侧栏宽度。
  - 左栏增加“当前上下文”，账号管理和角色列表改为折叠式管理区。
  - 右栏在角色页仅保留倒计时、待办和可折叠历史。
- 主题 token 继续收敛：
  - 新增 `field-control / surface-table / context-card / banner-* / tone-*`。
  - 关键主页面减少对深色 utility 的依赖。
- 截图回归：
  - 新增命令：`npm run capture:ui-baseline:phase13`
  - Phase 13 截图目录：`artifacts/ui-baseline/phase13/`
- 本次校验：
  - `npm run typecheck` 通过
  - `npm run test:unit` 通过
  - `npm run build` 通过
  - `node scripts/ui-baseline-phase0-capture.mjs --out-dir artifacts/ui-baseline/phase13` 通过（16 张截图）
- 结论：
  - 中间主区的聚焦度已明显提升。
  - 工坊默认视图开始真正接近“只保留做装主任务”的目标。

### UI 改造 Phase 9 完成（角色页减重 + 设置页压缩 + 侧栏细修）
- 角色操作页重排：
  - 顶部改为 4 个紧凑摘要卡：`清空奥德预估 / 职业与装分 / 本周收益 / 回廊剩余`
  - `角色资料` 与 `快捷操作` 拆成并列分组，不再做成长串按钮区
  - `奥德能量 / 微风商店记录 / 变换记录` 重排为更轻的双列区块
- 设置页分组压缩：
  - 调整为 `收益与阈值 / 优先级偏好 / 构建信息 / 深渊回廊参数` 四块
  - 保存、导出、导入操作统一放在页头右侧
- 左右侧栏细修：
  - 侧栏卡片内边距和列表行距继续下降
  - 右栏“操作中心”按钮支持换行，窄宽度下不再挤压
- 截图回归：
  - 新增命令：`npm run capture:ui-baseline:phase9`
  - Phase 9 截图目录：`artifacts/ui-baseline/phase9/`
- 本次校验：
  - `npm run typecheck` 通过
  - `npm run test:unit` 通过
  - `npm run build` 通过
  - `node scripts/ui-baseline-phase0-capture.mjs --out-dir artifacts/ui-baseline/phase9` 通过（16 张截图）
- 结论：
  - 角色页与设置页已经明显脱离“参数墙”观感。
  - 目前剩余的主要密度问题，已经集中到角色总览卡本身。

### UI 改造 Phase 8 完成（导航左移 + 中栏净化）
- 顶部“控制台导航”已移出中栏，改到左侧栏：
  - 目录导航、版本号、检查更新现在固定在左边。
  - 中栏首屏不再被导航卡占据。
- 中栏顶部统计卡已移除：
  - 删除 `可远征角色 / 清空奥德预估 / 每日使命未清 / 每周指令未清` 四张卡。
  - 首屏现在更接近“打开即进入当前内容”。
- 工坊顺序修复：
  - `做装模拟器` 保持在 `专业模式` 上方。
  - 原因是此前 `order-*` 与默认顺序叠加导致折叠区跑到前面。
- 做装模拟器材料区改为默认展开：
  - 材料明细与库存修正不再是折叠项。
  - 直接展示，减少一次点击。
- 截图回归：
  - 新增命令：`npm run capture:ui-baseline:phase8`
  - Phase 8 截图目录：`artifacts/ui-baseline/phase8/`
- 本次校验：
  - `npm run typecheck` 通过
  - `npm run test:unit` 通过
  - `npm run build` 通过
  - `node scripts/ui-baseline-phase0-capture.mjs --out-dir artifacts/ui-baseline/phase8` 通过（16 张截图）
- 结论：
  - 中栏已经明显更干净。
  - 左栏开始承担目录角色，整体结构更接近你想要的 ChatGPT 式工作台。

### UI 改造 Phase 7 完成（主流程聚焦 + 语义色修正）
- 主按钮降反差：
  - 从近黑按钮改为柔和石墨灰按钮。
  - 在浅色主题里不再产生过强视觉打点。
- 角色总览语义色调整：
...
- 工坊默认主流程重构：
  - 默认首屏仅保留“装备制作”主卡。
  - `OCR抓价器 / 市场分析器 / 历史价格管理 / 库存管理` 全部下沉到“专业模式”展开区。
- 截图脚本同步修复：
  - `专业模式` 折叠后，基线脚本会自动展开再抓取 OCR / 市场 / 库存卡片。
  - 新增命令：`npm run capture:ui-baseline:phase7`
  - Phase 7 截图目录：`artifacts/ui-baseline/phase7/`
- 本次校验：
  - `npm run typecheck` 通过
  - `npm run test:unit` 通过
  - `npm run build` 通过
  - `node scripts/ui-baseline-phase0-capture.mjs --out-dir artifacts/ui-baseline/phase7` 通过（16 张截图）
- 结论：
  - 工坊使用路径已更符合“先做装备，再按需展开价格管理”的行为逻辑。
  - 角色总览已从“标签墙”开始转向“重点提醒面板”。
...
- 次级信息下沉：
  ...
  改为次级信息区。
- 材料明细区已折叠：
  - 材料名联动、库存修改、价格修改、取价来源查看仍保留。
- 截图脚本增强：
  - 抓工坊截图前会先自动执行一次默认 `运行模拟`。
  - 新增命令：`npm run capture:ui-baseline:phase4`
  - Phase 4 截图目录：`artifacts/ui-baseline/phase4/`
- 本次校验：
  - `npm run typecheck` 通过
  - `npm run test:unit` 通过
  - `npm run build` 通过
  - `node scripts/ui-baseline-phase0-capture.mjs --out-dir artifacts/ui-baseline/phase4` 通过（16 张截图）
- 结论：
  - 工坊主流程已基本形成“选配方 -> 抓价格 -> 看结论”的低噪音默认体验。
  - 下一步进入 `Phase 5`：做全局一致性收尾与交互细节统一。

### UI 改造 Phase 3 完成（市场分析减负）
- 市场分析器默认首屏已压缩为 4 块摘要：
  - 伺服器最近价格
  - 世界最近价格
  - 趋势方向
...
- 展开区仍保留：
  ...
  - 周期信号明细
- 截图回归：
  - 新增命令：`npm run capture:ui-baseline:phase3`
  - Phase 3 截图目录：`artifacts/ui-baseline/phase3/`
- 本次校验：
  - `npm run typecheck` 通过
  - `npm run test:unit` 通过
  - `npm run build` 通过
  - `node scripts/ui-baseline-phase0-capture.mjs --out-dir artifacts/ui-baseline/phase3` 通过（16 张截图）
- 结论：
  - 市场分析器默认区已接近“最近价格 + 趋势方向 + 异常提醒”的目标。
  - 下一步进入 `Phase 4`：压缩做装模拟器结果区，强化“是否值得做”的一眼判断。
...
- 工坊 UI 热修：
  - 修复“做装模拟器”卡片被 `order-*` 样式覆盖导致仍在下方的问题。
  - 调整后顺序为：做装模拟器 -> OCR抓价器 -> 市场分析器 -> 库存管理。
- 发布前校验：
  - `npm run test:smoke` 通过
  - `npm run dist:win` 通过
...
- OCR 热修：
  - 修复安装版 ONNX 模型从 `app.asar` 读取失败导致抓价不可用的问题。
  - 新增模型路径回退到 `app.asar.unpacked`，并补充打包解包配置。
- 工坊体验优化：
  - “做装模拟器”卡片调整至工坊页最上方。
- 发布前校验：
  - `npm run test:smoke` 通过
...
### v2.1.0 发布（架构重构收口 + 热修合集）
- 发布版本升级至 `v2.1.0`。
- 完成重构收口（冻结结构性继续拆分，进入稳定迭代）：
  - renderer 侧完成 view/hooks/actions 分层聚合，页面组合与联动路径收敛。
  - main/workshop-store 侧完成 domain 化拆分，pricing/simulation/ocr/catalog/store 责任边界清晰化。
  - IPC 调用桥接统一由 shared contract 驱动，错误口径与 payload 校验一致。
- 本轮热修功能：
  - 圣域开箱奥德消耗调整为 `80`。
  - 做装模拟器材料点击可同步市场分析器 + 历史价格管理卡片（无页面跳转/抖动）。
  - 修复做装模拟器手动价格与 OCR 抓价覆盖冲突，手动输入价格优先保持。
  - 合并“运行模拟”与“保存并重算”按钮为单一“运行模拟”。
  - 新增角色星标（`★`）与待办排序加权。
  - 历史价格管理新增“一键清空全部历史价格”。
- 发布前校验：
  - `npm run typecheck` 通过
...
### v2.1.0 覆盖发布（更新链路修正）
- 自动更新链路修正：
  - 新增主进程统一更新服务 `app-updater`，启动自动检查并收敛异常处理。
  - 新增前端“检查更新”入口，发现新版本后下载完成自动重启安装。
- 版本可见性增强：
  - 工具栏新增当前版本常驻显示（快速区分旧版/新版）。
- Release 资产规范修正：
  - 安装包资产名与 `latest.yml` 路径统一，修复更新元数据与下载资产不一致问题。

## 2026-02-18

### v2.0.0 正式上线（安装版 + 解压版）
- 发布版本升级至 `v2.0.0`，进入正式上线阶段。
- OCR 引擎迁移为本地 ONNX 推理（免 Python / 免 PaddleOCR 环境部署）：
  - 默认走 ONNX 本地识别链路，降低用户安装门槛。
  - 交易行双列价格保持“伺服器 / 世界”并行导入。
- OCR 导入链路增强（名称纠错与误识别防护）：
  - 增加简繁转换与领域词组归一化（例如“純的 -> 純淨的”、“龍族片 -> 龍族鱗片”）。
  - 增加限定词识别与修复（燦爛 / 純淨 / 高純度）。
  - 增加“閃耀前缀道具”忽略规则，避免误识别价格写入。
  - 名称区识别放大倍率提升，改善小字漏识别问题。
- 价格质量风控补强：
  - 异常判定从“仅依赖历史中位数”升级为“历史偏离 + 分类绝对阈值”并行。
  - 装备类新增硬阈值/可疑阈值（低价误识别可直接拦截）。
- 内置目录修订：
  - `達人閃耀的法珠` 更名为 `達人閃耀的真實法珠`。
  - 增加旧名到新名的迁移映射，兼容历史数据与价格记录。

## 2026-02-17

### 今日追加（模拟器取价策略 + 星标持久化 + 3.2 反推重构）
- 工坊 Phase 4.3 / 4.4 下线：
  - 已删除“双市场价差扫描 / 行动队列”功能及相关前端计算逻辑，避免无效干扰。
- 制作模拟器材料价格导入规则调整：
  - 材料默认单价改为优先取“伺服器/世界”两市最低价（若只有单列价格则回退单列）。
  - 材料表新增“默认取价”列，明确当前默认来自哪个市场。
- 星标记忆修复：
  - 星标物品改为本地持久化，程序关闭重开后会保留。
- 背包逆向推导（3.2）重构：
  - 由“列出大量配方”改为“按材料反推 / 综合背包关联推荐”。
  - 支持输入材料关键词并选择目标材料，筛出关联配方并按关联分排序。
  - 每条建议同步给出补差材料、单次补差金额、预算建议次数、预算潜在利润。
  - 缺价格材料口径改为“直接配方材料”，不再展示间接展开材料。
  - 新增“关联偏好”选择（平衡/覆盖率优先/利润优先/可直接制作优先）。
  - 关联权重参数“用户手动微调”暂记为后续待办：先观察真实用户反馈，再决定是否开放高级参数。

### 今日完成（工坊体验收口 + 行情联动）
- 交易行 OCR 抓价框升级为“可拖动、可缩放”动态模式：
  - 名称框/价格框均支持可视化调整，不再依赖固定尺寸。
  - 行数识别改为可配置并适配不同 UI 缩放（如小 UI 8 行、中 UI 7 行）。
- 世界交易所/伺服器交易所列位识别修复：
  - 利用表头识别左右列语义，避免“世界价格被识别成伺服器价格”的错列写入。
- 做装模拟增强：
  - 新增“制作后成品售价”输入项，可直接录入目标成品当前售价参与计算。
- 数据清理补齐：
  - 已删除“未分类”中的历史误识别乱码物品。
- 行情中心自动化与搜索增强：
  - 选中材料/装备后自动触发历史查询，移除手动“查询历史”依赖。
  - 物品搜索改为全量搜索，不再受当前大类/子类限制。
  - 修复搜索框清空后需重新点击才能继续输入的问题。
- 界面“无抖动/不下滑”体验修复：
  - 删除历史价格、点击星标、点击“管理历史价格”等操作不再触发页面下滑。
  - 行情查询后移除不必要的弹跳/滚动行为，交互更连续。
- 行情中心与历史管理打通：
  - 新增从行情中心直达“历史价格管理”的入口，并自动带入当前物品。
  - 右侧删除历史价格后，左侧价格曲线与指标即时同步刷新，无需二次查询或切换周期。
- 周期性波动提示联动增强：
  - 可从波动提示直接联动到行情中心查看该物品的具体价格曲线细节。
- 星标物品能力上线：
  - 支持为重点关注材料/装备打星，用于置顶/高亮跟踪。

### 工坊 Phase 4.2（利润门槛驱动 + 可解释信号）
- 将趋势阈值默认值与最小值统一提升为 `15%`，避免 `8%` 级别信号误导操作：
  - 信号规则输入低于 `15%` 将被拒绝（前端）/钳制（后端）。
  - 历史低阈值存量配置会在归一化时自动提升到 `15%`。
- 信号评估改为“可解释”输出：
  - 每条信号新增 `confidenceScore`（置信分）与 `reasons`（判定依据）。
  - 汇总返回新增 `effectiveThresholdRatio`（实际生效阈值）。
- 行情中心 UI 同步增强：
  - 新增阈值“输入值/生效值”并排展示。
  - `Phase 4` 进/出货列表与触发列表新增“置信分 + 判定依据”展示，便于快速验证策略质量。
...
## 2026-02-16（夜间 OCR 架构复盘）

### 今日完成（OCR 抓价稳定性 + 性能主线）
- 自动巡航截图流程去抖动：
  - 自动模式不再在截图前后 `hide/show` 浮窗，减少窗口状态切换与驱动握手压力。
  - 手动截图/校准仍保留可控隐藏逻辑，避免影响调试。
- 新增 OCR 安全模式（默认开启）：
  - 配置项 `safeMode` 打通 `types -> renderer -> automation -> store` 全链路。
  - 安全模式下强制 CPU 优先，规避 GPU 路径对显卡驱动状态的潜在干扰。
  - UI 新增“安全模式（CPU）/性能模式（允许非 CPU）”切换与本地持久化。
- OCR 执行器从“每轮冷启动”升级为“常驻 worker”：
  - 主路径：常驻 Python 子进程 + stdin/stdout JSON 请求队列。
  - 容错：worker 不可用时自动回退到一次性命令调用，保证可用性不回退。
  - 保护：新增请求超时、stdout/stderr 缓冲上限、异常时统一清理 pending 请求。
  - 生命周期：应用退出时显式销毁 OCR worker，避免悬挂进程。
- 浮窗能力增强：
  - 除状态外，新增展示“最近成功抓价条目（物品 + 价格 + 市场列）”。
  - 限制显示前 N 条并显示剩余计数，避免信息淹没。
- 中文名匹配误写修正（重点问题收敛）：
  - 收紧 OCR 名称候选生成，不再随意裁剪前缀，降低“纯净前缀被吞”误匹配风险。
  - 新增限定词前缀保护：若疑似“限定名折叠到基础名”，直接跳过并告警。
  - 增加繁简归一映射，提升繁中环境下名称键匹配稳定性。
  - 新增“已知物品词典单字纠错”：对单字误识别、且候选唯一时自动纠正（如近形字）。

### 今日完成（对外行为变化）
- OCR 默认策略变更：
  - 默认更偏“稳态优先”，即使牺牲部分峰值速度，也优先避免黑屏/切屏干扰。
- OCR 成功反馈增强：
  - 不切回主程序也可在浮窗看到抓价结果，缩短验证回路。
- OCR 导入策略更保守：
  - 对高风险歧义名优先“跳过并提示”，不再“猜一个写进去”。

### 性能复盘（为什么之前会慢）
- 旧路径瓶颈：
  - 每轮 OCR 均执行 `spawn python -> import paddleocr -> 初始化引擎`，初始化成本重复支付。
  - 一轮抓价内存在多次 OCR 调用（名称区、价格区、表头/回退流程），叠加耗时明显。
- 新路径收益点：
  - 常驻 worker 将“解释器启动 + 模型初始化”摊平到首次请求。
  - 后续请求主要消耗在推理本身与图像裁切，轮询总耗时可显著下降。

### 工程取舍（可复用到下个项目）
- 稳定性优先级：
  - 首选“确定可控”的 CPU 路径，再给用户开放性能模式作为可选项。
- 架构优先级：
  - 先做“可观测 + 可降级 + 可回退”，再追求极限吞吐。
- 识别策略优先级：
  - 先做“宁可漏，不可错写”；套利类工具里误写成本高于漏识别。

### 面向下一项目（PoE2 市场通货抓取/套利）迁移建议
- 推荐基础流水线：
  - `截图 -> ROI 切分 -> OCR -> 名称/价格标准化 -> 置信度与歧义判定 -> 入库 -> 套利引擎`。
- OCR 层建议：
  - 把“字典纠错 + 歧义拦截 + 繁简归一”做成独立模块，游戏项目可复用。
  - 保留“跳过并告警”机制，不把低置信识别直接写入行情库。
- 技术路线建议：
  - 短期：沿用当前 worker 模式快速迭代业务正确性。
  - 中期：转 ONNX 本地推理（去 Python 依赖），实现“安装即用 + 轻运维”。
  - 长期：将 OCR 与套利核心解耦，OCR 可替换，多策略可并行 A/B。
- Rust 取舍建议：
  - Rust 非常适合数据清洗、时序聚合、套利规则回测与在线分析（性能/并发/稳定性好）。
  - OCR 若走 ONNX，Rust 可直接承接推理与后处理；若短期仍依赖 Python，可先做 Rust 主程序 + OCR sidecar。

### 发布与产品化影响（当前状态）
- 当前发布包可直接运行主程序，但 OCR 仍依赖本机 Python + PaddleOCR 环境。
- 若目标是“用户安装后无需额外环境”，需进入下一阶段：
  - 方案 A：随包内置 Python 运行时与模型（改造快，体积变大）。
  - 方案 B：OCR 改 ONNX 本地推理（工程量更高，但最终体验更干净）。
...
### 今日完成（OCR 抓价卡片精简 + 热键流修复）
- “市场工作台：OCR 抓价导入”卡片完成简约化：
  - 删除手动截图路径 / OCR 语言 / 手动文本导入 / 图标抓取与写入规则等非必要入口。
  - 仅保留热键抓价主流程 + 交易行预设 + 手动拖拽框选“名称框 / 价格框”。
- 交易行预设改为默认 `1440p`，并记住上次选择（localStorage 持久化）。
- OCR 默认语言固定为繁中 `chi_tra`，不再暴露顶部语言参数。
- 快捷抓价默认截图延迟从 `1200ms` 下调为 `600ms`。
- 双价格列 OCR 增加快速解析路径：
  - 先对整块价格区域做一次 OCR，再按左右列分词拆分。
  - 若有效行不足，自动回退旧的双区块识别，保证稳定性。

### 当日追加修复（按测试反馈）
- 修复“名称歧义，已跳过”的误判：
  - 仅对“模糊匹配命中”的场景执行歧义拦截。
  - 对“精确命中物品名”的场景不再误跳过（覆盖已录入材料被误拦截问题）。
- 默认热键从 `Shift+F8` 调整为 `Shift+F1`，并同步更新 UI 推荐文案与失败提示。
...
- 发布收口：
  - 准备 `v2.2.0`（极简工作台版）发布。
  - 收口角色总览、角色操作、做装模拟与市场工具的当前极简 UI 方案。
- 逻辑修复：
  - 回廊同步改为同步上限，相关展示不再固定按 `3` 计算。
  - 快捷录入 `每日使命` 改回增量优先，保留直接设总完成作为辅助动作。
  - `每周指令书 / 深渊下层指令 / 深渊中层指令` 分类并入 `周常`。
- 视觉修补：
  - 总览任务行增加轻量比例填充。
  - 深渊周常名称明确为“深渊下层指令 / 深渊中层指令”。
## 2026-03-12

- `v2.2.1` 热修：
  - 圣域规则从旧的“挑战/开箱”双逻辑改为两个独立圣域周本。
  - 新增 `圣域：卢德莱` 与 `圣域：侵蚀净化所` 两张独立卡片。
  - 两张圣域卡统一为单次完成扣 `80` 奥德。
- 文档沉淀：
  - 新增 `UI_STYLE_ARCHITECTURE_GUIDE.md`，供其他项目复用当前 UI 架构与风格。
