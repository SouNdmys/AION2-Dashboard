...
export interface BuildWorkshopSampleSeedStateDeps {
  createId: () => string;
  normalizeRecipeInputs: (raw: unknown) => WorkshopRecipeInput[];
...
export function buildWorkshopSampleSeedState(
  state: WorkshopState,
  nowIso: string,
  deps: BuildWorkshopSampleSeedStateDeps,
): WorkshopState {
  ...
  WORKSHOP_SAMPLE_RECIPES.forEach((seed) => {
    ...
    if (existing) {
      ...
      const nextRecipe: WorkshopRecipe = {
        ...
        inputs: deps.normalizeRecipeInputs(inputs),
        updatedAt: nowIso,
      };
      if (index >= 0) {
  ...
  return {
    ...
    items,
    recipes: nextRecipes,
    prices: nextPrices.slice(-deps.priceHistoryLimit),
