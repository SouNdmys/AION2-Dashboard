    ...
    <div className="mt-2.5 space-y-2.5">
      <section className="resource-band">
        <div className="grid gap-3 xl:grid-cols-[minmax(0,1.45fr)_minmax(0,0.95fr)] xl:items-center">
          <div>
            <div className="resource-band-header">
              <div>
                <p className="panel-kicker !tracking-[0.08em]">Energy</p>
                <h3 className="resource-band-title">奥德能量</h3>
                <p className="resource-band-subtitle">基础能量优先消耗，补充能量兜底。</p>
              </div>
      ...
      <div className="grid gap-3 xl:grid-cols-2">
        <section className="resource-band h-full">
          <div>
            <p className="panel-kicker !tracking-[0.08em]">Breeze Shop</p>
            <h3 className="resource-band-title">微风商店记录</h3>
            <p className="resource-band-subtitle">按伺服器统一上限记录本周已购买数量。</p>
          </div>
          <div className="mt-3 grid gap-2 md:grid-cols-2">
            ...
            <label className="space-y-1">
              <span className="text-xs text-slate-500">未知缝隙挑战券(刻印) 剩余 {selectedShopDailyDungeonTicketPurchaseRemaining}/{UNKNOWN_CHALLENGE_TICKET_SERVER_LIMIT}</span>
              <select className="field-control" value={shopUnknownChallengeTicketUsedInput} onChange={(event) => onShopUnknownChallengeTicketUsedInputChange(event.target.value)} disabled={busy}>
                {buildCountOptions(0, UNKNOWN_CHALLENGE_TICKET_SERVER_LIMIT, shopUnknownChallengeTicketUsedInput).map((value) => (
                  <option key={`shop-unknown-${value}`} value={value}>未知缝隙挑战券 已购 {value}</option>
                ))}
              </select>
            </label>
            <label className="space-y-1">
            ...
            <label className="space-y-1 md:col-span-2">
              <span className="text-xs text-slate-500">深渊重铸补充券(刻印) 剩余 {selectedShopAbyssReplenishRemaining}/{ABYSS_REPLENISH_TICKET_SERVER_LIMIT}，用于卢德莱 +1 挑战/+1 开箱</span>
              <select className="field-control" value={shopAbyssReplenishUsedInput} onChange={(event) => onShopAbyssReplenishUsedInputChange(event.target.value)} disabled={busy}>
                {buildCountOptions(0, ABYSS_REPLENISH_TICKET_SERVER_LIMIT, shopAbyssReplenishUsedInput).map((value) => (
                  <option key={`shop-abyss-${value}`} value={value}>深渊重铸补充券 已购 {value}</option>
                ))}
              </select>
            </label>
          </div>
        ...
        <section className="resource-band h-full">
          <div>
            ...
            <h3 className="resource-band-title">奥德兑换记录</h3>
            <p className="resource-band-subtitle">伺服器统一次数池，按周维护兑换消耗。</p>
          </div>
